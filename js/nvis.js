...
window.onload = function() {
    renderer = new NvisRenderer();

    // let stream0 = renderer.loadStream(["images/triangle.0000.png", "images/triangle.0001.png", "images/triangle.0002.png"]);
    // let stream1 = renderer.loadStream(["images/triangle.0003.png", "images/triangle.0004.png", "images/triangle.0005.png"]);
	...
	// differenceStream.addInputStream(stream1);
    // renderer.addWindow(differenceStream);
    ...
    let _getUI = function (streamId, streams)
    {
        ...
        if (!_bFileStream)
        {
            for (let inputId = 0; inputId < _shader.getNumInputs(); inputId++)
            {
                ...
                sEl.setAttribute("onchange", "renderer.streamUpdateInput(" + streamId + ", " + inputId + ")");
    ...
    let _onKeyPress = function (event)
    {
        ...
        switch (event.key)
        {
            ...
            case 'D':
                ...
                {
                    renderer.loadShader("glsl/difference.json", true);
                }
                break;
            case 'w':
...
//				_uiPopup.style.display = (_uiPopup.style.display == "none" ? "block" : "none");
    ...
    let _loadShader = function (jsonFileName, bNewStreamAndWindow = false)
    {
        let xhr = new XMLHttpRequest();
        ...
        xhr.onload = function () {
            if (this.status == 200 && this.responseText !== null)
            {
                ...
                if (bNewStreamAndWindow && _streams.length >= newShader.getNumInputs())
                {
